...
import { listMonthLeaderPerformance } from '@/api/common'
import { listYearLeaderPerformance } from '@/api/common'
...
const listMonthLeaderData = async () => {
  try {
    ...
    const response = await listMonthLeaderPerformance(params)
    if (response && response.data) {
      // 2. 赋值原始数据
      cityPerformanceData.value = response.data
...
const listYearLeaderData = async () => {
  try {
    ...
    const response = await listYearLeaderPerformance(params)
    if (response && response.data) {
      // 2. 赋值原始数据
      cityPerformanceData.value = response.data
