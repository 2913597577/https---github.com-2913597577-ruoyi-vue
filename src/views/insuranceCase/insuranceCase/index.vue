        ...
        <div class="flex items-center">
          <el-col :span="1.5">
            <el-button type="primary"  icon="Search" @click="handleSearch"
              v-hasPermi="['insuranceCase:insuranceCase:search']">筛选
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button   icon="Refresh" @click="getList"
              v-hasPermi="['insuranceCase:insuranceCase:refresh']">刷新
            </el-button>
          </el-col>
        </div>
...
const handleDelete = async (row?: InsuranceCaseVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除保险记录 编号为:"' + _ids + '",客户名称为:"'+ customerName +'"的数据项？').finally(() => loading.value = false);
