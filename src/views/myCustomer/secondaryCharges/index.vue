    ...
    <div class="dialog-content">
      <el-form  ref="customerTransferFormRef" :model="form" :rules="rules" label-width="120px hide-required-asterisk">
       ...
       <!-- 流转单标题 -->
          ...
          <h1 class="blue-title">二次收费客户流转单</h1>
        </div>
...
const data = reactive<PageData<CustomerTransferForm, CustomerTransferQuery>>({
  ...
  rules: {
   ...
   /*  serviceType: [
    ...
    secondDevelopmentType: [
      { required: true, message: "请选择二次收费类型", trigger: "change" }
    ],
    serviceStart: [
...
const getList = async () => {
  loading.value = true;
  queryParams.value.isSecondaryCharge=1; // 只查询初次收费的流转单
...
const handleCompanyChange = (companyId: string) => {
  ...
  if (selectedCompany) {
    ...
    addressModel.value = {
      province: getCodeByName(selectedCompany.province || '') || '370000',
      city: getCodeByName(selectedCompany.city || ''),
      district: getCodeByName(selectedCompany.district || '')
    };
    // form.value 的省市区字段可以保留（用于提交时的名称存储）
    form.value.province = selectedCompany.province || '';
    form.value.city = selectedCompany.city || ''
    form.value.district = selectedCompany.district || '';
