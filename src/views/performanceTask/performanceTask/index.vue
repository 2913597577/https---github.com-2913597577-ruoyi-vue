        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            ...
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['performanceTask:performanceTask:edit']">修改</el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="danger" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['performanceTask:performanceTask:remove']">删除</el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
const handleView = async (row: PerformanceTaskVO) => {
  ...
  viewDialog.title = `【${row.legalSupportName}】的业绩任务记录`;

  try {
    // 调用列表接口，传入 legalSupportId 作为筛选条件
    const res = await listPerformanceTask({ 
      legalSupportId: row.legalSupportId, // 直接传入 ID，非数组
      pageNum: 1,
