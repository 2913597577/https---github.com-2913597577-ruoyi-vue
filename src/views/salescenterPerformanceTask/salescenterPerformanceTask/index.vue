  ...
  <div class="p-2">
    ...
    <el-card shadow="never">
      ...
      <el-table 
       ...
       :row-class-name="tableRowClassName"
        ...
        <el-table-column label="销售员工" align="center" prop="salesCenterName" />
    ...
    <el-dialog v-model="searchDialogVisible" title="筛选" width="900px" append-to-body draggable>
      ...
      <div v-show="showSearch" class="mb-[10px]">
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            <el-form-item label="销售员工" prop="salesCenterId">
              <el-select filterable v-model="queryParams.salesCenterId" placeholder="请选择销售人员" clearable style="width: 100%;"
              @keyup.enter="handleQuery">
            <el-option v-for="lawyer in lawyerList" :key="lawyer.userId"
              :label="lawyer.nickName + '(' + lawyer.userName + ')'" :value="lawyer.userId" filterable></el-option>
          </el-select>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body draggable>
      <el-form ref="PerformanceTaskFormRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="销售员工" prop="salesCenterId">
           <el-select filterable v-model="form.salesCenterId" placeholder="请选择销售人员" clearable style="width: 100%;"
...
import { listPerformanceTask, getPerformanceTask, delPerformanceTask, addPerformanceTask, updatePerformanceTask } from '@/api/salescenterPerformanceTask/salescenterPerformanceTask';
import { PerformanceTaskVO, PerformanceTaskQuery, PerformanceTaskForm } from '@/api/salescenterPerformanceTask/salescenterPerformanceTask/types';
...
const initFormData: PerformanceTaskForm = {
  id: undefined,
  salesCenterId: undefined,
  salesCenterName: undefined,
  taskMonth: undefined,
...
const data = reactive<PageData<PerformanceTaskForm, PerformanceTaskQuery>>({
  ...
  rules: {
    ...
    salesCenterId: [
      { required: true, message: "销售员工id不能为空", trigger: "blur" }
    ],
    taskMonth: [
...
const handleLegalSupportChange = (userId: string) => {
  if (userId) {
    ...
    if (selectedLawyer) {
      ...
      form.value.salesCenterName = selectedLawyer.nickName;
    }
  } else {
    // 清空选择时重置相关字段
    form.value.salesCenterName = undefined;
  }
}
/** 查询业绩任务列表 */
const getList = async () => {
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('salescenterPerformanceTask/salescenterPerformanceTask/export', {
    ...queryParams.value
  }, `SalesCenterPerformanceTask_${new Date().getTime()}.xlsx`)
}
...
// 选中多行之后，颜色高亮显示
const tableRowClassName = ({ row }: { row:  PerformanceTaskVO }) => {
