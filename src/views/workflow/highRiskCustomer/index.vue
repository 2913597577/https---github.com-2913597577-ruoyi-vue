    ...
    <el-card shadow="never">
      <template #header>
        ...
        <div class="flex items-center">
          ...
          <el-col :span="1.5">
            <el-button v-hasPermi="['workflow:highRiskCustomer:export']" type="warning" plain icon="Download" @click="handleExport">导出</el-button>
          </el-col>
        </div>
        <div class="flex items-center">
          <el-col :span="1.5">
            <el-button type="primary"  icon="Search" @click="handleSearch"
              v-hasPermi="['workflow:highRiskCustomer:search']">筛选
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button   icon="Refresh" @click="getList"
              v-hasPermi="['workflow:highRiskCustomer:refresh']">刷新
            </el-button>
          </el-col>
        </div>
          ...
          <template #default="scope">
            <el-row :gutter="10" >
              <el-col :span="1.5" v-if="scope.row.status === 'draft' || scope.row.status === 'cancel' || scope.row.status === 'back'">
                <el-button v-hasPermi="['workflow:highRiskCustomer:edit']" size="small" link type="warning" icon="Edit" @click="handleUpdate(scope.row)"
                  >修改</el-button
                >
              </el-col>
              <el-col :span="1.5" v-if="scope.row.status === 'draft' || scope.row.status === 'cancel' || scope.row.status === 'back'">
                <el-button v-hasPermi="['workflow:highRiskCustomer:remove']" size="small" link type="danger" icon="Delete" @click="handleDelete(scope.row)"
                  >删除</el-button
                >
              </el-col>
            </el-row>
...
// 选中多行之后，颜色高亮显示
const tableRowClassName = ({ row }: { row: DcHighRiskCustomerVO }) => {
